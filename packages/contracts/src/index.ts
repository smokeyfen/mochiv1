...
export const HUMAN_REALISM_VERSION = 'HUMAN_REALISM_V1' as const;
export type HumanRealismVersion = typeof HUMAN_REALISM_VERSION;
export interface HumanRealismBehavior { approachBehavior:string; gripAndContactBehavior:string; actionExecutionBehavior:string; postActionSettleBehavior:string; cameraBehavior:string; }
export interface HumanRealismScene { sceneId:string; index:1|2|3|4; primaryAction:ActionId; startState:CanonicalPhysicalState; endState:CanonicalPhysicalState; behavior:HumanRealismBehavior; }
export interface HumanRealismGlobalConstraints { cameraFamily:'SMARTPHONE_POV'; reviewerFaceVisibility:'FORBIDDEN'; preserveHandIdentity:true; preserveDominantHand:true; noUncontractedSecondHand:true; onePrimaryPhysicalAction:true; noUncontractedCutOrReset:true; noProductTeleportation:true; noHandProductPenetration:true; noImpossibleGrip:true; maintainContactContinuity:true; noRoboticMotion:true; subtleHandheldCameraOnly:true; actionCompletionPriority:true; exactCanonicalStates:true; }
export interface HumanRealism4ScenePlan { schemaVersion:SchemaVersion; realismVersion:HumanRealismVersion; productId:string; sourceEvidenceVersion:string; canonicalAssetIds:readonly string[]; continuity:GlobalContinuityState; referenceReadiness:'READY'|'LIMITED'; referenceLimitations:readonly ReferenceLimitationCode[]; globalConstraints:HumanRealismGlobalConstraints; scenes:readonly HumanRealismScene[]; }
const realismPlanKeys=['schemaVersion','realismVersion','productId','sourceEvidenceVersion','canonicalAssetIds','continuity','referenceReadiness','referenceLimitations','globalConstraints','scenes'] as const;const realismSceneKeys=['sceneId','index','primaryAction','startState','endState','behavior'] as const;const realismBehaviorKeys=['approachBehavior','gripAndContactBehavior','actionExecutionBehavior','postActionSettleBehavior','cameraBehavior'] as const;
const nonBlankContract=(value:unknown):value is string=>typeof value==='string'&&value.trim().length>0;
