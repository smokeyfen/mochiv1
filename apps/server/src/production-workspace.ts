...
export type FinalAcceptanceBlockerCode='SCENE_QC_NOT_PASS'|'VIDEO_TECHNICAL_INVALID'|'SEQUENCE_QC_NOT_PASS'|'STALE_SEQUENCE'|'STALE_CANDIDATE'|'SNAPSHOT_MISMATCH'|'DIALOGUE_MISMATCH'|'VOICE_MISMATCH'|'PROMPT_BUDGET_INVALID'|'KEY_POINT_PLAN_INVALID'|'REFERENCE_BINDING_INVALID'|'PAIRWISE_QC_NOT_PASS'|'GLOBAL_CONTINUITY_NOT_PASS';
...
export const DELIVERY_PACKAGE_V1='DELIVERY_PACKAGE_V1' as const;
export type DeliveryOutputName='scene-01.mp4'|'scene-02.mp4'|'scene-03.mp4'|'scene-04.mp4'|'key-points.txt';
export interface SafeDeliveryManifest { readonly version:typeof DELIVERY_PACKAGE_V1; readonly status:'NOT_READY'|'READY_FOR_DELIVERY'|'DELIVERED'; readonly outputs:readonly {readonly filename:DeliveryOutputName;readonly mimeType:'video/mp4'|'text/plain; charset=utf-8'}[]; }
type Candidate = { readonly snapshotId:string; readonly sceneId:string; readonly candidateAssetId:string; readonly attempt:number; video?:SceneQcVideoMediaV1; technical?:ValidatedVideoTechnicalMetadataV1; report?:SceneQcReportV1; };
type BoundSequence = { readonly view:SafeSequenceView; readonly snapshotId:string; readonly candidateAssetIds:readonly [string,string,string,string]; };
type DeliveryReceipt = { readonly snapshotId:string; readonly candidateAssetIds:readonly [string,string,string,string]; readonly finalAcceptance:SafeFinalAcceptanceView; readonly delivered:boolean; };
type Active = { readonly snapshot:ProductionSnapshotV1; readonly productName:string; readonly anchors:readonly [SceneAnchorV1,SceneAnchorV1,SceneAnchorV1,SceneAnchorV1]; readonly requests:readonly [FlowProductionRequestV1,FlowProductionRequestV1,FlowProductionRequestV1,FlowProductionRequestV1]; readonly references:readonly BrowserReferenceMedia[]; readonly candidates:Map<string,Candidate>; sequence?:BoundSequence; finalAcceptance?:SafeFinalAcceptanceView; delivery?:DeliveryReceipt; };
...
export class ProductionWorkspaceService {
  ...
  async uploadAndQc(snapshotId:string,sceneId:string,candidateAssetId:string,file:{readonly type:string;readonly bytes:Uint8Array}):Promise<SafeSceneView> {
    const active=this.requireActive(); const candidate=active.candidates.get(sceneId); if(snapshotId!==active.snapshot.snapshotId||!candidate||candidate.candidateAssetId!==candidateAssetId||candidate.snapshotId!==snapshotId||candidate.video||!file.type.startsWith('video/')||file.bytes.length===0) throw new ProductionWorkspaceError('INVALID_OR_STALE_VIDEO');
    ...
    let technical:ValidatedVideoTechnicalMetadataV1; try { technical=validateVideoTechnicalMetadataV1(await this.videoMetadataInspector.inspect({mimeType:file.type,bytes:file.bytes})); } catch(error) { if(error instanceof VideoTechnicalValidationError) throw new ProductionWorkspaceError('VIDEO_TECHNICAL_INVALID'); throw new ProductionWorkspaceError('VIDEO_TECHNICAL_INVALID'); }
    this.invalidateQcDependentState(active);
    const video:SceneQcVideoMediaV1={sceneId,candidateAssetId,mimeType:file.type,dataBase64:Buffer.from(file.bytes).toString('base64')};
    ...
    let report:SceneQcReportV1; try { report=await evaluateSceneQcV1({candidate:{candidateVersion:GENERATED_SCENE_CANDIDATE_V1,sceneId,candidateAssetId,dialogue:anchor.dialogue,voiceIdentityId:anchor.voiceIdentityId,lifecycleStatus:'QC_PENDING'},anchor,productionRequest:active.requests[index]!,video,references,intelligence:this.runtimeConfiguration.provider()!}); } catch { throw new ProductionWorkspaceError('SCENE_QC_FAILED'); }
    candidate.video=video; candidate.technical=technical; candidate.report=report;
  ...
  async runSequence(snapshotId:string):Promise<SafeSequenceView> { const active=this.requireActive(); if(snapshotId!==active.snapshot.snapshotId) throw new ProductionWorkspaceError('INVALID_OR_STALE_VIDEO'); const entries=active.anchors.map(anchor=>active.candidates.get(anchor.sceneId)!); if(entries.some(entry=>entry.report?.result!=='SCENE_QC_PASS'||!entry.video||!entry.technical)) throw new ProductionWorkspaceError('CONTINUITY_QC_FAILED'); const referenceMedia=active.anchors.map(anchor=>active.references.map(reference=>({sceneId:anchor.sceneId,assetId:reference.assetId,mimeType:reference.mimeType,dataBase64:reference.dataBase64}))) as unknown as [readonly SceneQcReferenceMediaV1[],readonly SceneQcReferenceMediaV1[],readonly SceneQcReferenceMediaV1[],readonly SceneQcReferenceMediaV1[]]; try { const result=await runManualCandidateSequenceQcV1({snapshot:active.snapshot,anchors:active.anchors,referenceMedia,sceneQcReports:entries.map(entry=>entry.report!) as [SceneQcReportV1,SceneQcReportV1,SceneQcReportV1,SceneQcReportV1],videos:entries.map(entry=>entry.video!) as [SceneQcVideoMediaV1,SceneQcVideoMediaV1,SceneQcVideoMediaV1,SceneQcVideoMediaV1],intelligence:this.runtimeConfiguration.provider()!}); const safe:SafeSequenceView={status:result.status==='FOUR_SCENE_SEQUENCE_PASS'?'FOUR_SCENE_SEQUENCE_PASS':'SEQUENCE_QC_FAILED',pairs:result.pairwiseAssessments,globalGates:result.globalGates,visualVariation:result.visualVariation!.status}; active.sequence={view:safe,snapshotId:active.snapshot.snapshotId,candidateAssetIds:this.activeCandidateAssetIds(active)}; active.finalAcceptance=this.evaluateFinalAcceptance(active); delete active.delivery; return safe; } catch { throw new ProductionWorkspaceError('CONTINUITY_QC_FAILED'); } }
  ...
  deliveryManifest(snapshotId:string):SafeDeliveryManifest { const active=this.requireActive(); if(snapshotId!==active.snapshot.snapshotId) throw new ProductionWorkspaceError('DELIVERY_NOT_READY'); const ready=this.deliveryReady(active); return {version:DELIVERY_PACKAGE_V1,status:ready?(active.delivery?.delivered?'DELIVERED':'READY_FOR_DELIVERY'):'NOT_READY',outputs:this.deliveryOutputs()}; }
  deliveryOutput(snapshotId:string,filename:DeliveryOutputName):{readonly mimeType:'video/mp4'|'text/plain; charset=utf-8';readonly bytes:Uint8Array} { const active=this.requireActive(); if(snapshotId!==active.snapshot.snapshotId||!this.deliveryReady(active)) throw new ProductionWorkspaceError('DELIVERY_NOT_READY'); if(filename==='key-points.txt') return {mimeType:'text/plain; charset=utf-8',bytes:Buffer.from(this.keyPointsText(active),'utf8')}; const index=(['scene-01.mp4','scene-02.mp4','scene-03.mp4','scene-04.mp4'] as const).indexOf(filename as 'scene-01.mp4'); if(index<0) throw new ProductionWorkspaceError('DELIVERY_OUTPUT_NOT_FOUND'); const candidate=active.candidates.get(active.anchors[index]!.sceneId); if(!candidate?.video||candidate.video.candidateAssetId!==candidate.candidateAssetId) throw new ProductionWorkspaceError('DELIVERY_NOT_READY'); return {mimeType:'video/mp4',bytes:Buffer.from(candidate.video.dataBase64,'base64')}; }
  recordDelivery(snapshotId:string):SafeDeliveryManifest { const active=this.requireActive(); if(snapshotId!==active.snapshot.snapshotId||!this.deliveryReady(active)||!active.delivery) throw new ProductionWorkspaceError('DELIVERY_NOT_READY'); active.delivery={...active.delivery,delivered:true}; return this.deliveryManifest(snapshotId); }
  private evaluateFinalAcceptance(active:Active):SafeFinalAcceptanceView { const blockers=new Set<FinalAcceptanceBlockerCode>(); if(validateProductionSnapshotV1(active.snapshot).length||active.anchors.length!==4) blockers.add('SNAPSHOT_MISMATCH'); active.anchors.forEach((anchor,index)=>{const candidate=active.candidates.get(anchor.sceneId);const request=active.requests[index]!;if(validateSceneAnchorAgainstSnapshot(anchor,active.snapshot).length||anchor.snapshotId!==active.snapshot.snapshotId)blockers.add('SNAPSHOT_MISMATCH');if(!candidate||candidate.snapshotId!==active.snapshot.snapshotId||candidate.sceneId!==anchor.sceneId||candidate.video?.candidateAssetId!==candidate.candidateAssetId)blockers.add('STALE_CANDIDATE');if(!candidate?.technical||candidate.technical.result!=='VIDEO_TECHNICAL_PASS')blockers.add('VIDEO_TECHNICAL_INVALID');if(candidate?.report?.result!=='SCENE_QC_PASS'||candidate.report.candidateAssetId!==candidate.candidateAssetId||candidate.report.sceneId!==anchor.sceneId)blockers.add('SCENE_QC_NOT_PASS');if(request.sceneId!==anchor.sceneId||request.dialogue!==anchor.dialogue)blockers.add('DIALOGUE_MISMATCH');if(request.nativeVoiceBinding.logicalVoiceIdentityId!==anchor.voiceIdentityId)blockers.add('VOICE_MISMATCH');if(request.durationSeconds!==8||request.aspectRatio!=='9:16'||request.promptUnicodeCharacterCount>3200)blockers.add('PROMPT_BUDGET_INVALID');if(request.referenceBindings.length!==anchor.referenceAssetIds.length||request.referenceBindings.some((binding,i)=>binding.sceneId!==anchor.sceneId||binding.logicalAssetId!==anchor.referenceAssetIds[i]))blockers.add('REFERENCE_BINDING_INVALID');const points=active.snapshot.productionContract.scenes[index]?.keyPoints;if(!points||points.length!==2||(index===0&&points[0]?.text!==active.productName))blockers.add('KEY_POINT_PLAN_INVALID');}); const sequence=active.sequence;if(!sequence||sequence.view.status!=='FOUR_SCENE_SEQUENCE_PASS')blockers.add('SEQUENCE_QC_NOT_PASS'); else if(sequence.snapshotId!==active.snapshot.snapshotId||!this.sameCandidateIds(sequence.candidateAssetIds,this.activeCandidateAssetIds(active))) blockers.add('STALE_SEQUENCE'); else {if(sequence.view.pairs.length!==3||sequence.view.pairs.some(pair=>pair.gates.some(gate=>gate.status!=='PASS')))blockers.add('PAIRWISE_QC_NOT_PASS');if(sequence.view.globalGates.some(gate=>gate.status!=='PASS'))blockers.add('GLOBAL_CONTINUITY_NOT_PASS');} return {version:'FINAL_ACCEPTANCE_V1',status:blockers.size===0?'FINAL_ACCEPTANCE_PASS':'FINAL_ACCEPTANCE_BLOCKED',blockerCodes:[...blockers]}; }
  private activeCandidateAssetIds(active:Active):[string,string,string,string] { return active.anchors.map(anchor=>active.candidates.get(anchor.sceneId)!.candidateAssetId) as [string,string,string,string]; }
  private sameCandidateIds(a:readonly string[],b:readonly string[]):boolean { return a.length===4&&b.length===4&&a.every((id,index)=>id===b[index]); }
  private invalidateQcDependentState(active:Active):void { delete active.sequence; delete active.finalAcceptance; delete active.delivery; }
  private deliveryOutputs():SafeDeliveryManifest['outputs'] { return [{filename:'scene-01.mp4',mimeType:'video/mp4'},{filename:'scene-02.mp4',mimeType:'video/mp4'},{filename:'scene-03.mp4',mimeType:'video/mp4'},{filename:'scene-04.mp4',mimeType:'video/mp4'},{filename:'key-points.txt',mimeType:'text/plain; charset=utf-8'}]; }
