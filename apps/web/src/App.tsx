...
import { buildProduction, connectGemini, disconnectGemini, replaceFailedCandidate, runFinalAcceptance, runSequence, runtimeStatus, uploadCandidate, ProductionClientError, type FinalAcceptanceView, type RuntimeStatus, type SceneView, type SequenceView } from './production-client';
...
function RuntimeSetup({ status, onStatus }: { readonly status: RuntimeStatus; readonly onStatus:(status:RuntimeStatus)=>void }) {
  ...
  const disconnect=async()=>{try{onStatus(await disconnectGemini());setError(undefined);}catch{setError('Gemini could not be disconnected.');}};
  return <section className="runtime-setup" aria-labelledby="runtime-title"><p className="status-label">Runtime Setup</p><h2 id="runtime-title">Connect Gemini</h2><div className="runtime-row"><span>Runtime status: <strong>{status}</strong></span><label>Gemini API Key<input aria-label="Gemini API Key" type="password" value={key} onChange={event=>setKey(event.target.value)}/></label><button type="button" className="primary-button" onClick={()=>void connect()} disabled={!key}>Connect</button><button type="button" className="secondary-button" onClick={()=>void disconnect()}>Disconnect</button></div><p>Connection is held only in this server process and clears on restart.</p>{error&&<p role="alert" className="analysis-error">{error}</p>}</section>;
}
...
function ProductionWorkspace({ readyInput, references, files, runtime }: { readonly readyInput:MochiProjectInput|null; readonly references:readonly ReferenceAssetState[]; readonly files:ReadonlyMap<string,File>; readonly runtime:RuntimeStatus }) {
  const [snapshotId,setSnapshotId]=useState<string>(); const [scenes,setScenes]=useState<SceneView[]>([]); const [sequence,setSequence]=useState<SequenceView>(); const [finalAcceptance,setFinalAcceptance]=useState<FinalAcceptanceView>(); const [message,setMessage]=useState<string>();
