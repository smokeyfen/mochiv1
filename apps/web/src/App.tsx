...
function RuntimeSetup({ status, onStatus }: { readonly status: RuntimeStatus; readonly onStatus:(status:RuntimeStatus)=>void }) {
  ...
  return <section className="runtime-setup" aria-labelledby="runtime-title"><p className="status-label">Runtime Setup</p><h2 id="runtime-title">Connect Gemini</h2><div className="runtime-row"><span>Runtime status: <strong>{status}</strong></span>{status==='GEMINI_READY'?<><span>Gemini API Key</span><strong>LOCKED</strong><button type="button" className="secondary-button" onClick={()=>void disconnect()}>Disconnect</button></>:<><label>Gemini API Key<input aria-label="Gemini API Key" type="password" value={key} onChange={event=>setKey(event.target.value)}/></label><button type="button" className="primary-button" onClick={()=>void connect()} disabled={!key}>Connect</button></>}</div><p>Connection is held only in this server process and clears on restart.</p>{error&&<p role="alert" className="analysis-error">{error}</p>}</section>;
}
