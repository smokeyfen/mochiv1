...
import { buildProduction, connectGemini, disconnectGemini, runSequence, runtimeStatus, uploadCandidate, ProductionClientError, type RuntimeStatus, type SceneView, type SequenceView } from './production-client';
...
function ProductionWorkspace({ readyInput, references, files, runtime }: { readonly readyInput:MochiProjectInput|null; readonly references:readonly ReferenceAssetState[]; readonly files:ReadonlyMap<string,File>; readonly runtime:RuntimeStatus }) {
  const [snapshotId,setSnapshotId]=useState<string>(); const [scenes,setScenes]=useState<SceneView[]>([]); const [sequence,setSequence]=useState<SequenceView>(); const [message,setMessage]=useState<string>();
