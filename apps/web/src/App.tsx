...
import { buildProduction, connectGemini, deliveryManifest, deliveryOutput, disconnectGemini, recordDelivery, replaceFailedCandidate, runFinalAcceptance, runSequence, runtimeStatus, uploadCandidate, ProductionClientError, type DeliveryManifest, type DeliveryOutputName, type FinalAcceptanceView, type ProductionDiagnostic, type RuntimeStatus, type SceneView, type SequenceView } from './production-client';
...
function ProductionWorkspace({ readyInput, references, files, runtime, analysisReceiptId, productionInputRevision, onWorkflowChange }: { readonly readyInput:MochiProjectInput|null; readonly references:readonly ReferenceAssetState[]; readonly files:ReadonlyMap<string,File>; readonly runtime:RuntimeStatus; readonly analysisReceiptId:string|null; readonly productionInputRevision:number; readonly onWorkflowChange:(state:ProductionWorkflowState)=>void }) {
  const [snapshotId,setSnapshotId]=useState<string>(); const [scenes,setScenes]=useState<SceneView[]>([]); const [sequence,setSequence]=useState<SequenceView>(); const [finalAcceptance,setFinalAcceptance]=useState<FinalAcceptanceView>(); const [delivery,setDelivery]=useState<DeliveryManifest>(); const [message,setMessage]=useState<string>(); const [stage,setStage]=useState<string>(); const [diagnostic,setDiagnostic]=useState<ProductionDiagnostic>(); const [building,setBuilding]=useState(false);
  useEffect(()=>{setSnapshotId(undefined);setScenes([]);setSequence(undefined);setFinalAcceptance(undefined);setDelivery(undefined);setMessage(undefined);setStage(undefined);setDiagnostic(undefined);setBuilding(false);},[productionInputRevision]);
