...
function ProductionWorkspace({ readyInput, references, files, runtime, factualRevision, creativeRevision, analyzeFoundation, onWorkflowChange }: { readonly readyInput:MochiProjectInput|null;readonly references:readonly ReferenceAssetState[];readonly files:ReadonlyMap<string,File>;readonly runtime:RuntimeStatus;readonly factualRevision:number;readonly creativeRevision:number;readonly analyzeFoundation:()=>Promise<string>;readonly onWorkflowChange:(state:ProductionWorkflowState)=>void }) {
  ...
  const runL1=async()=>{if(!readyInput||runtime!=='GEMINI_READY'||layers.L1.status==='RUNNING')return;setLayers(current=>({...current,L1:{status:'RUNNING'}}));try{const foundationId=await analyzeFoundation();setLayers({L1:{status:'PASS',id:foundationId},L2:{status:'READY'},L3:{status:'LOCKED'},L4:{status:'LOCKED'}});setScenes([]);}catch(error){setLayers(current=>({...current,L1:failure('L1',error),L2:{status:'LOCKED'},L3:{status:'LOCKED'},L4:{status:'LOCKED'}}));}};
  const runL2=async()=>{const foundationId=layers.L1.id;if(!readyInput||!foundationId||layers.L2.status==='RUNNING')return;setLayers(current=>({...current,L2:{status:'RUNNING'},L3:{status:'LOCKED'},L4:{status:'LOCKED'}}));try{const result=await createSceneBlueprint(foundationId,readyInput.creativeDirection);setScenes(result.scenes);setLayers(current=>({...current,L1:{status:'PASS',id:foundationId},L2:{status:'PASS',id:result.blueprintId},L3:{status:'READY'},L4:{status:'LOCKED'}}));}catch(error){setLayers(current=>({...current,L2:failure('L2',error)}));}};
  const runL3=async()=>{const blueprintId=layers.L2.id;if(!blueprintId||layers.L3.status==='RUNNING')return;setLayers(current=>({...current,L3:{status:'RUNNING'},L4:{status:'LOCKED'}}));try{const result=await createFinalizedScript(blueprintId);setScenes(result.scenes);setLayers(current=>({...current,L3:{status:'PASS',id:result.scriptId},L4:{status:'READY'}}));}catch(error){setLayers(current=>({...current,L3:failure('L3',error)}));}};
  const runL4=async()=>{const scriptId=layers.L3.id;if(!scriptId||layers.L4.status==='RUNNING')return;setLayers(current=>({...current,L4:{status:'RUNNING'}}));try{const result=await compileProduction(scriptId);setScenes(result.scenes);setSnapshotId(result.snapshotId);setLayers(current=>({...current,L4:{status:'READY_FOR_FLOW',id:result.readyId}}));}catch(error){setLayers(current=>({...current,L4:failure('L4',error)}));}};
