...
function ProductionWorkspace({ readyInput, references, files, runtime, analysisReceiptId, onPlanChange }: { readonly readyInput:MochiProjectInput|null; readonly references:readonly ReferenceAssetState[]; readonly files:ReadonlyMap<string,File>; readonly runtime:RuntimeStatus; readonly analysisReceiptId:string|null; readonly onPlanChange:(hasPlan:boolean)=>void }) {
...
export function App() {
  ...
  return (
    <main className="shell">
      ...
      <nav className="workflow" aria-label="Production workflow">{[['Setup',readyInput!==null],['Product Analysis',analysisState==='READY'&&analysisReceiptId!==null],['Production Plan',hasPlan],['Scene QC',hasPlan&&false],['Delivery',false]].map(([label,complete],index)=>{const current=!complete&&([readyInput!==null,analysisState==='READY'&&analysisReceiptId!==null,hasPlan,false,false] as boolean[]).slice(0,index).every(Boolean);return <div key={label as string} className={complete?'workflow-complete':current?'workflow-current':'workflow-not-ready'}><span>{index+1}</span><strong>{label as string}</strong><small>{complete?'COMPLETE':current?'CURRENT':'NOT_READY'}</small></div>;})}</nav>
      ...
      <ProductionWorkspace readyInput={readyInput} references={referenceAssets} files={filesByAssetId.current} runtime={runtime} analysisReceiptId={analysisReceiptId} onPlanChange={setHasPlan} />
    </main>
  );
}
