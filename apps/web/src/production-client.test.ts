...
it('preserves only a canonical safe production build stage', async()=>{
  ...
  await expect(buildProduction(project,files)).rejects.toMatchObject({code:'PRODUCTION_BUILD_FAILED',stage:'R6_BOUNDED_REPLAN'} satisfies Partial<ProductionClientError>);
});
...
it('keeps generic production build errors generic when no safe canonical stage exists', async()=>{
  ...
  await expect(buildProduction(project,files)).rejects.toMatchObject({code:'PRODUCTION_BUILD_FAILED',stage:undefined} satisfies Partial<ProductionClientError>);
});
