...
- Reviewer face is forbidden; every scene has exactly one semantic Primary Action.
...
- BGM is `NONE`; VFX is `NONE`; SFX is physically grounded or `NONE`.
...
- `packages/reasoning/src/v1-2/action-library.ts`: expanded action definitions and eligibility.
- `packages/reasoning/src/v1-2/state-v2.ts`: physical plus bounded functional state resolution.
...
- Produce version constants `PRODUCT_FOUNDATION_V1_2`, `SCENE_BLUEPRINT_V1_2`, `FINALIZED_SCRIPT_V1_2`, `PRODUCTION_COMPILED_V1_2`, `PRODUCTION_READY_V1_2`, `STATE_ENGINE_V2`, `HUMAN_REALISM_V2`, `SFX_PLAN_V2`, and `SCENE_EXECUTION_CONTRACT_V2`.
- Produce strict types for reference purpose, commercial score, Semantic Pair, seeded decision trace, physical/functional state, camera/focus composition, two-sentence dialogue, dialogue-derived Key Points, effects, and execution lineage.
...
- Define `PRODUCTION_ELIGIBILITY_POLICY_V1_2` to deny `UNTESTED` and `AVOID`, default every newly represented action to `UNTESTED`, and define `MAX_SCENE_REPLAN_ATTEMPTS_V1_2 = 2`.
...
Add tests that construct four valid scene contracts, then independently mutate version, scene count/order, duration, aspect ratio, camera family, reviewer-face rule, Primary Action count, dialogue sentence count, Semantic Pair mapping, Key Point count, Scene 1 Product Name fields, BGM/VFX, seed, and provider-only fields. Assert each mutation fails. The four-scene audit must also reject mixed source/seed/version lineage, broken adjacent physical or functional state, reference-variant mutation, and missing useful camera diversity.
...
### Task 4: Expanded Action Library + State Engine V2 + eligibility

**Review gate:** Broad bounded actions are available only when truth, affordance, hand-count, state, and production eligibility all pass.
...
- Produce `getEligiblePrimaryActionsV1_2(profile, state, capability, policy): readonly EligiblePrimaryActionV1_2[]` using the versioned capability map, initialized `UNTESTED` for every V1.2 action.
- Produce `selectPrimaryActionV1_2(eligible, creativeSeed, sceneNamespace): SeededSelectionResultV1_2<EligiblePrimaryActionV1_2>` using the Gate 3 weighted selector only after eligibility.
...
- Encode each action's one semantic goal, hand-count rule, affordance requirements, preconditions, deterministic effects, and permissible grounded sound events.

- [ ] **Step 1: Write failing action-table and state-transition tests**

Use table-driven tests for every action. Assert functional actions fail without matching Product Truth-backed affordances; two hands fail unless supported/required; each scene has one Primary Action; every adjacent `END N === START N+1`; hidden reset/refill/reassembly/variant change fails. Assert `UNTESTED`/`AVOID` fail production, risk precedence is deterministic, replan selects only already eligible grounded actions, seed and locked fields survive, state/risk recompute after each attempt, and the bounded attempt limit cannot loop.
...
Keep V1 `ActionId`, `ACTION_SEMANTICS`, `resolveSceneStates`, capability evidence, and fast-track behavior unchanged. New V1.2 eligibility must not interpret “broad support” as SAFE. Actual newly represented actions remain blocked until trusted evidence satisfies the locked promotion policy or a separately approved policy admits them; explicit SAFE/RISKY maps are test fixtures only and never production evidence.
...
export function selectPrimaryActionV1_2(eligible: readonly EligiblePrimaryActionV1_2[], creativeSeed: string, sceneNamespace: string): SeededSelectionResultV1_2<EligiblePrimaryActionV1_2>;
...
```bash
git add packages/reasoning/src/v1-2 packages/reasoning/src/v1-2-action-library.test.ts packages/reasoning/src/v1-2-state-v2.test.ts packages/reasoning/src/v1-2-action-eligibility.test.ts packages/reasoning/src/v1-2-scene-risk-v2.test.ts packages/reasoning/src/v1-2-replan-v1-2.test.ts packages/reasoning/src/index.ts
git commit -m "feat(reasoning): add V1.2 action and state engine"
```

### Task 5: Global Camera/Focus + Human Realism V2

**Review gate:** Four-scene composition is usefully diverse and realistic, and every realism field is retained for downstream compilation.
...
- Produce `composeGlobalCameraFocusV1_2(plan, statePlan, creativeSeed): GlobalCameraFocusPlanV1_2`.
- Produce `planHumanRealismV2(input, intelligence): Promise<HumanRealism4ScenePlanV2>` with output limited to behavior fields and deterministic binding to action/state/composition.
...
Assert composition considers role + insight + action + state; rejects cinematic orbit/drone/gimbal, obscured actions, redundant four-scene composition, face visibility, undeclared second hands, impossible grip, and state mutation. Include one-hand and valid two-hand fixtures.
...
Keep ordinary subtle handheld behavior. Bind every generated realism string to its scene/action/state/composition and reject missing, extra, stale, or summarized behavior fields.
...
**Review gate:** Every scene has exactly two timed Vietnamese sentences and exactly two later-derived Key Points with exact semantic pairing.
...
- Produce `finalizeDialogueV2(input, profile, intelligence): Promise<DialoguePlanV2>`.
- Produce `deriveKeyPointsV2(dialoguePlan, context): KeyPointsPlanV2` only after dialogue validation.
- Sentence 1 maps Pair 1; sentence 2 maps Pair 2. Scene 1 sentence 1 contains the exact Product Name.
...
Reject one or three sentences, wrong language, swapped pair mapping, missing/altered Scene 1 Product Name, Key Points created from unvalidated/stale dialogue, new facts in Key Points, wrong eight-point count, and non-name secondary points outside the 5–7-word target.
...
**Review gate:** Sound is physically justified, bounded to the one Primary Action, or canonically `NONE`; BGM/VFX cannot change.
...
- Produce `compileSfxPlanV2(action, affordance, state): SfxPlanV2`.
- Each sound event has a stable kind, action phase, visible physical cause, and restrained intensity; no event is returned when physical justification is absent.

- [ ] **Step 1: Write failing grounded-sound tests**

Cover click for a grounded switch/closure, press sound only for a supported actuator, pour/dispense sound only with supported content state, `NONE` for silent presentation, and rejection of music, voice replacement, cinematic whoosh, unsupported off-screen sound, BGM other than `NONE`, or VFX other than `NONE`.
...
Derive events only from the selected action definition, matched affordance, and resolved state transition. Do not call intelligence or accept free-form sound design.

```ts
export function compileSfxPlanV2(action: EligiblePrimaryActionV1_2, affordance: ProductAffordanceV1_2 | null, state: ResolvedSceneStateV2): SfxPlanV2;
```
...
- Produce `compileFlowScenePromptV2(anchor): CompiledFlowScenePromptV2` containing lossless action/state/camera/realism/dialogue/key-point/reference/SFX semantics.

- [ ] **Step 1: Write a lossless field matrix test first**

Create one sentinel value per production-relevant field and assert exact equality at R8, reloaded P0, SceneAnchor, Flow request, and prompt semantic sections. Mutate or omit every field class and assert fail-closed rejection. Assert provider IDs occur only in the provider request and cannot re-enter Core snapshots.
...
- L2 persists `creativeSeed`, seeded decisions, Semantic Pairs, insight/action selection, exact `ActionCapabilityMapV1_2`, exact `ProductionEligibilityPolicyV1_2`, initial State V2/Risk V2, bounded-replan trace/result, final State V2/Risk V2, camera/focus, and Human Realism V2.
- L3 persists two-sentence dialogue, dialogue-derived Key Points, timing, and SFX Plan V2.
...
Test content-addressed idempotence, immutable predecessor IDs, server-owned 128-bit seed format, deliberate-regenerate new seed, same-failure retry seed preservation, receipt loss/mismatch failure, stale/corrupt/mixed-version rejection, no partial artifact after failure, and no V1.2 compiled artifact until the Gate 1 `auditFourSceneExecutionSetV2` returns PASS for all four cards. Reload must recompute and compare capability map, eligibility policy, initial/final State V2, initial/final Risk V2, and replan trace; stale policy/map/risk fails. All final risk scenes must be `READY` before camera/realism, L3, or L4.
...
L2 execution order is fixed: seeded eligible action selection → initial State Engine V2 → initial Scene Risk V2 → bounded replan when non-READY → recomputed final State Engine V2 → recomputed final Scene Risk V2 with all scenes `READY` → Global Camera/Focus → Human Realism V2. Any failure stops before the next stage and commits no L2 artifact.
...
- Display four progressive Scene Cards with insight, Hook/CTA approach, action/hand count, functional state, camera/focus, two dialogue sentences, two Key Points, realism, references, SFX, and audit status.
...
Reject unknown/extra/mixed-version response fields. Test factual invalidation of L1–L4, creative invalidation of L2–L4, stale completion suppression, regenerate versus retry seed semantics, four-card completeness, reviewer-face warning visibility, and absence of a Flow action before all-card PASS.
...
- Matrix includes simple presentation plus grounded open/close, remove/replace cap, press, switch, pour, dispense, apply, assemble, eligible two-hand, and corresponding unsupported/contradictory cases.

- [ ] **Step 1: Write failing matrix and generation-order tests**

Assert every fixture's exact eligibility/result code. Positive functional fixtures use explicit test-only SAFE capability maps; repository production capability remains unchanged and the same cases are BLOCKED with the real all-UNTESTED state. Inject a Flow driver spy and prove zero generation calls for one failing card, mixed lineage, lost execution field, broken continuity, duplicate Primary Action, unsupported affordance, invalid reference role, camera violation, dialogue/key-point violation, or SFX violation.
...
The server pre-Flow gate certifies, rather than duplicates, the exact Gate 9 compiled artifact. It composes the already-passing Gate 1 structural audit with exact L1→L4 persisted-lineage reload and Gate 8 lossless boundary checks. It emits the sole `READY_FOR_FLOW` authority only for four exact V1.2 Scene Execution Contracts with a shared source lineage and seed, returns bounded fail-closed reason codes, and never calls Flow.
...
Confirm by source inspection that Core has no provider fields, L4 has no `IntelligenceProvider` or generation dependency, all four-card failures precede Flow driver access, and every Scene Execution Contract V2 field reaches the prompt/request losslessly.
