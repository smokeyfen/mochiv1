...
- **Supporting feature/function references** and **functional-state references** must bind to the same reviewed variant as canonical identity.
- **Supporting variant references** may represent a supported other variant, but never replace canonical identity or mutate the reviewed variant. Their different `productVariantId` is derived deterministically from trusted product/source and asset-fingerprint identity; it is never provider-authored.

Reference-purpose classification is a derived, validated authority; V1.2 does not require manual front/side/back classification. A bounded enum-only L1 assessment examines each existing canonical Product Evidence asset exactly once with its runtime image, then a deterministic compiler binds the result to the R2 committed context and copies the exact trusted ordered R1 asset ID/MIME/SHA-256 fingerprints into `ReferencePurposesV1_2`. Bytes and MIME are rehashed and compared before intelligence; wrong bytes, wrong MIME, reordered assets, stale receipts, and logical-ID relabeling fail closed. The assessment may report only a bounded purpose plus `SAME_REVIEWED_VARIANT`, `SUPPORTED_OTHER_VARIANT`, `CONFLICT`, or `AMBIGUOUS`; it cannot author product prose, fingerprints, or variant IDs. `SUPPORTED_OTHER_VARIANT` is valid only for `SUPPORTING_VARIANT`. Missing, duplicate, unknown, contradictory, or incomplete results also fail closed. Any supporting reference that conflicts with canonical product identity is excluded or blocks planning. The retained fingerprint binding is the Gate 2 content lineage for later persistence comparison; it does not replace the server receipt check.
