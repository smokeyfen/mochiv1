...
Task 3 audited frontend/transport corrective is **IMPLEMENTATION PASS / awaiting ChatGPT follow-up source and diff audit**. Each browser L1-L4 request now captures a per-layer operation generation, the applicable exact input revisions, and its predecessor opaque ID. Factual invalidation cancels the authority of every in-flight layer result; creative invalidation cancels L2-L4; replacement L2 cancels old L3/L4; and replacement L3 cancels old L4. Stale success and failure completions are ignored and cannot write current browser lineage, scenes, snapshot, or failure state. Creative Direction now exposes controlled Audience, Shooting Context, Reviewer Persona, Tone, Voice Style, Voice Gender, and Voice Region values. Gender and SOUTH/NORTH Region are independent selected-card controls; clicking an already-selected value is a no-op. Locked validation still fails closed. The strict layered client accepts only exact-shape `GEMINI_NOT_CONFIGURED`, `INVALID_SETUP`, and `PRODUCT_ANALYSIS_STALE` non-layer error bodies in addition to the existing bounded layer diagnostics; unknown codes, undeclared fields, and malformed bodies become `INVALID_RESPONSE`. Verification passes: typecheck; 368 tests/0 failures; web build; diff check. No backend engine/schema, R1-R8, P0, SceneAnchor, Flow, QC, Final Acceptance, Delivery, or compatibility build semantics changed. No live Gemini or Flow call occurred.

Layered HTTP + Frontend Workflow Task 3 is **IMPLEMENTATION PASS / awaiting ChatGPT independent source and diff audit**. Four explicit routes now compose the unchanged persisted engines: `POST /api/production/foundation` resolves the exact existing Product Analysis receipt and invokes L1 through `trustedProductEvidence`; `/blueprint` consumes only `foundationId`; `/script` consumes only `blueprintId`; and `/compile` consumes only `scriptId`. One fresh browser Analyze Product macro action makes exactly one Product Evidence, one Product Truth, and one Reference Assessment call. L4 has no IntelligenceProvider, Flow driver, generation, Gemini, or automatic QC path; its exact reloaded P0 snapshot, four SceneAnchors, and four compiled Flow requests activate the existing manual QC workspace, with candidate IDs allocated only there. Reference media remains transient process memory and does not enter L1-L4, P0, Core, or Reasoning artifacts. Safe transport returns only opaque IDs, strict progressive scene views, and allowlisted L1/L2 runtime or L3/L4 phase/stage diagnostics. The UI's explicit lineage states are `LOCKED | READY | RUNNING | PASS | FAIL | STALE | NEEDS_REBUILD | NEEDS_REANALYSIS | READY_FOR_FLOW`; factual edits invalidate L1-L4, while creative edits retain the exact L1 ID and invalidate only L2-L4. Independent retries retain exact predecessor IDs. The same four Scene Cards mature from L2 skeletons through L3 dialogue/key points to L4 prompts and `READY_FOR_FLOW`. The new dark navy/slate UI uses restrained semantic teal, green, amber, and red LED state accents with visible keyboard focus and reduced-motion support. Existing manual Flow instructions, upload, Scene QC, Sequence QC, Final Acceptance, and exact five-file Delivery remain unchanged; `/api/production/build` and `createProductionRuntime().run()` remain compatibility authorities. Verification passes: typecheck; 352 tests/0 failures; web build; diff check. No live Gemini, Flow, Saydi, or generation occurred. Next: ChatGPT independent source/diff audit, then manual browser L1 → L2 → L3 → L4 E2E. Do not generate Flow until all four final Scene Card screenshots are audited.

Layered Orchestration Task 2B — L4 Production Compile is **IMPLEMENTATION PASS / awaiting ChatGPT independent diff audit**. The deterministic `runProductionCompile` boundary accepts only `scriptId`; its dependency contract contains `layerArtifactStore`, `productionSnapshotStore`, and optional deterministic compiler test seams, with no `IntelligenceProvider`. It validates and loads exact L3→L2→L1 lineage before invoking unchanged R8, then uses unchanged P0 create → reload → exact equality/integrity → `validateProductionSnapshotV1`. The exact reloaded snapshot is canonical. L4 compiles and validates exactly four ordered SceneAnchors and exactly four existing Flow requests with the unchanged PICK_UP, HOLD, ROTATE_SLOW, HOLD spine, exact L3 dialogue, immutable logical voice, ordered logical reference IDs, 8-second/9:16 format, `NONE` effects, and existing prompt budgets. Synthetic `manual-l4-validation-*` Flow attachment IDs exist only in the transient request view and never enter Core, Reasoning, P0, L1/L2/L3, `PRODUCTION_READY_V1`, or provider execution state; no driver or generation function is reachable. The thin durable server authority is content-addressed `pr_<sha256>` with `{ schemaVersion, readyVersion, readyId, scriptId, snapshotId, status:'READY_FOR_FLOW', compileProof:{ sceneCount:4, scenes:[bounded hashes/logical compile metadata] } }`. Atomic/idempotent persistence and load-time validation reject corrupt content or IDs. Bounded L4 diagnostics expose only layer, phase, and the applicable R8 or downstream compile stage. Regression coverage proves upstream/P0/anchor/Flow/budget/ready-persistence failures commit no ready authority, preserve exact L1/L2/L3 bytes, and allow an isolated same-script retry to commit one stable ready artifact. The old one-click runtime remains passing. Verification passes: typecheck; 351 tests/0 failures; web build; diff check. No live Gemini, Flow, Saydi, generation, browser automation, retry loop, promotion, frontend, QC, acceptance, or delivery work occurred. Next: ChatGPT independent diff audit. Do not live-test Gemini or Flow.

Layered Orchestration migration Task 1 is **IMPLEMENTATION PASS / awaiting ChatGPT independent diff audit**. `PRODUCT_FOUNDATION_V1` is the durable L1 authority: `pf_<sha256>` binds project ID, exact factual `ProductInput`, ordered logical reference MIME/SHA-256 fingerprints without media bytes, validated Product Evidence, and the exact committed R2 context. `SCENE_BLUEPRINT_V1` is the durable L2 authority: `sb_<sha256>` binds the foundation ID, exact Creative Direction, ActionCapabilityMap/current production-eligibility policy, and the complete R3 continuity, final R4 plan, R5 state, R6 risk, and R7-A realism artifacts. The validators call the locked stage validators and deterministically recompute R2 Commit, R5, and R6; stale product/reference, lineage, creative, eligibility, continuity, plan, state, risk, and realism inputs fail closed. Both stores use canonical UTF-8 JSON, content-addressed IDs, same-root temporary files, atomic no-overwrite publication, idempotent saves, and load-time validation/integrity verification. Neither artifact stores Base64/reference bytes, credentials, provider/runtime metadata, raw prompts, or raw model responses. A failed L2 commits no blueprint and its explicit retry consumes the unchanged persisted L1 without any R1/R2 call; no automatic retry was added. Exact clean mocked calls are L1 = 3 (R1/R2-A/R2-B), L2 = 3 (R3/R4/R7-A), and the preserved one-click path = 9. The unchanged all-UNTESTED + `SIMPLE_ACTION_FAST_TRACK_V1` path persists PICK_UP, HOLD, ROTATE_SLOW, HOLD with four R6 READY scenes. `PRODUCTION_SNAPSHOT_V1`, the old `/api/production/build` path, all downstream stages, and the frontend remain in place. Verification passes: typecheck; 340 tests/0 failures; web build; diff check. No live Gemini, Flow, Saydi, generation, or promotion occurred. Next if PASS: ChatGPT independent diff audit, then Task 2 builds L3 Script Finalization and L4 Production Compile. Do not runtime test or generate Flow.
...
- Layered Orchestration L4 Production Compile: **IMPLEMENTATION PASS / awaiting ChatGPT independent diff audit**; deterministic `scriptId → P0 → four SceneAnchors → four Flow compiles → PRODUCTION_READY_V1`, with zero intelligence and zero generation calls
...
## Product Analysis Receipt + Browser Workflow

`PRODUCT_ANALYSIS_RECEIPT_V1` is a bounded, server-process-memory receipt store for successfully validated Product Analysis. It returns only sanitized evidence, opaque unpredictable `analysisReceiptId`, and receipt version. The server binds a receipt with SHA-256 to the exact ProductInput factual state and the ordered logical reference asset IDs, MIME types, and bytes. It never returns media, digests, provider metadata, raw model output, credentials, or paths. The store is FIFO-bounded, supports multiple current receipts, and intentionally loses all receipts on restart.

Browser Production Build sends the current project, files, and opaque receipt ID. The server independently resolves the receipt and validates its exact factual/byte binding; changes to name, details, category, or any reference make it stale, while audience, shooting context, voice, reviewer mode, and AUTO tone do not. A stale/unknown receipt fails safe as `PRODUCT_ANALYSIS_STALE` and never calls Product Evidence intelligence. For a current browser receipt, R1 remains a trace stage but deterministically revalidates trusted server-owned evidence; Product Evidence model calls during Build are **0**. Historical/PRE-F1 runtime composition remains model-backed R1.

The browser is now a responsive five-step workspace: Setup, Product Analysis, Production Plan, Scene QC, Delivery. Setup validity is continuously derived without a manual Validate action. Connected Gemini is compact and `LOCKED`; analysis defaults to a compact receipt-bound summary with expandable details; Production Plan has a real loading state and safe friendly stage errors with canonical details collapsed; scenes use primary hierarchy and a Flow three-step block. Manual E2E remains paused before Flow spend. Next: **MAJOR MANUAL V1 E2E from Analyze Product → Production Plan → Scene 1 Flow canary.** Gemini live = 0; Flow = 0; Saydi = 0 for this implementation.
...
MAJOR MANUAL V1 E2E runtime-boundary corrective: the Runtime Setup UI has explicit blank-entry and `LOCKED` states, keeping no credential input or value in the ready DOM and clearing local entry state on Connect, Disconnect, and a not-configured status. Runtime credentials remain process-memory-only. A failed build may return only `{ ok:false, error:{ code:'PRODUCTION_BUILD_FAILED', stage:<canonical PreF1RuntimeStage> } }`; only a retained `ProductionRuntimeError.stage` can populate that optional field, and trace/provider detail never crosses HTTP. The browser preserves that validated stage and renders `Production build failed at <stage>.`; missing or invalid stages retain the generic error. Verification: `npm run typecheck` PASS; `npm test` PASS (297 tests); no Gemini, Flow, Saydi, or generation call occurred.

MAJOR MANUAL V1 E2E Product Evidence grounding corrective: Product Evidence now treats references as visual authority only. Unsupported material/hidden-construction wording fails validation unless Product Details or a readable target-label note explicitly supports the material; uncertain composition must remain uncertainty and cannot duplicate a material certainty. The only claim sources remain `USER_INPUT` and `REFERENCE_EVIDENCE`: Product Details claims retain `USER_INPUT` and no image binding, while `REFERENCE_EVIDENCE` requires direct visible support and cannot carry suitability, gift, occasion, use-case, interaction, durability, safety, age, efficacy, or performance claims. Asset IDs remain binding-only and fail validation if they enter user-facing evidence prose, with the browser decoder independently applying the same validator. Product Truth receives only validated evidence and no Creative Direction input. Verification: `npm run typecheck` PASS; `npm test` PASS (307 tests); no Gemini, Flow, Saydi, or generation call occurred. Next if PASS: resume MAJOR MANUAL V1 E2E from Product Analysis.
