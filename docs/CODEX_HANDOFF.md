...
Task 7A Production Controls is IMPLEMENTATION PASS at `793dcaeb6340e9e706fd07624ab3957e56fe530e`. Creative Direction now has only fixed Audience and Shooting Context selects and two mutually exclusive NỮ/NAM voice cards; browser-bound hidden defaults retain the canonical reviewer/persona, AUTO role tone, `review`, and `SOUTH` contract fields. Selective repair replaces only a failed scene candidate with a new candidate ID and incremented attempt while preserving the same P0, anchor, prompt, dialogue, voice, and references; it invalidates only the repaired scene result plus sequence and final acceptance. Deterministic `FINAL_ACCEPTANCE_V1` is the final authority: four current scene PASS reports, a current four-scene PASS sequence with every pairwise and global critical gate PASS, exact dialogue and logical voice, valid 8-second/9:16/prompt/reference/key-point bindings are required. WARN-only Presentation Dynamics or Visual Variation does not block. No provider call is made by final acceptance. Task 7B Delivery Center + final browser E2E readiness is next; only after it completes does the major manual V1 E2E begin.
